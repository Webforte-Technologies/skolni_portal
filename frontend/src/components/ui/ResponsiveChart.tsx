...
export const ResponsiveChart: React.FC<ResponsiveChartProps> = ({
  ...
  const renderBarChart = useCallback((ctx: CanvasRenderingContext2D, chartData: any[], area: any) => {
  ...
  const renderLineChart = useCallback((ctx: CanvasRenderingContext2D, chartData: any[], area: any) => {
    const maxValue = Math.max(...chartData.map(d => d.value));
    ...
    ctx.stroke();
  }, [showValues, isMobile, colors]);

  const renderPieChart = useCallback((ctx: CanvasRenderingContext2D, chartData: any[], area: any, isDoughnut: boolean) => {
  ...
  const renderLegend = useCallback((ctx: CanvasRenderingContext2D, chartData: any[], area: any) => {
    const legendItemHeight = isMobile ? 15 : 20;
    const legendItemSpacing = isMobile ? 8 : 12;
    const itemsPerRow = Math.floor(area.width / (isMobile ? 80 : 100));
    let currentRow = 0;
    let currentCol = 0;

    chartData.forEach((item) => {
      const x = area.x + (currentCol * (isMobile ? 80 : 100));
      const y = area.y + (currentRow * (legendItemHeight + legendItemSpacing));
  ...
  }, [data, chartDimensions, type, showLegend, isMobile, title, colors, renderBarChart, renderLineChart, renderPieChart, renderLegend]);
